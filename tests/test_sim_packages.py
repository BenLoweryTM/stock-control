...
def run_la_instance(instance):
    ...
    df_run_infos["Starting Inv. Store"] = [
        [d[i][0] for i in range(instance["stores"])]
        for d in df_run_infos["Starting Inv. Store"]
    ]
    df_run_infos["Ending Inventory Store"] = [
        [d[i][0] for i in range(instance["stores"])]
        for d in df_run_infos["Ending Inventory Store"]
    ]
    df_run_infos["Starting Inv. Warehouse"] = [
        d[0] for d in df_run_infos["Starting Inv. Warehouse"]
    ]
    df_run_infos["Ending Inventory Warehouse"] = [
        d[0] for d in df_run_infos["Ending Inventory Warehouse"]
    ]
    return df_run_infos
